...
\stitle{Implementation Details.}  The SCP storage mechanism and optimal join plan generation(support both labelled and unlabelled queries) are strictly implemented according to \cite{Lai2016} and the optimized cost model we propose in Section \ref{sec:opt}. Here, given a join plan $J$, we will show the details of how to implement \gencliqjoin in \timely dataflow. 

\sstitle{(1) Building \timely Dataflow.} The building \timely dataflow procedure is shown in Algorithm \ref{alg:build_dataflow}. This algorithm is to compute the join operations round by round to get the final matches $R(q)$ in stream.\\
...
\ForAll{$j \in J$}{
    ...
    \If{$j.lg$ is a join unit}{
        \State{$LStream = I[i].to\_stream()$;}\\
        \State{$i \leftarrow i + 1$;}
    }
    \Else{
        \State{$LStream = StreamStack.Pop()$;}
    }
    \If{$j.rg$ is a join unit}{
        \State{$RStream = I[i].to\_stream()$;}\\
        \State{$i \leftarrow i + 1$;}
    }
    \Else{
    \State{$RStream = StreamStack.Pop()$;}
    }
    \State{$RstStream =  \code{BatchJoin}(LStream, RStream, j)$;}\\
